...
pack_Wgt(){
    ...
    clean_operator
    multi_thread_pack 12
    for wgt in $WGTLIST;do
        read -u 100
        {
            check_Suite $wgt
            if [ $? -eq 0 ];then
                $CTS_DIR/tools/build/pack.py -t wgt -s $SUITE_DIR -d $TIZEN_TESTS_DIR --tools=$CTS_DIR/tools
...
pack_Xpk(){
    for xpk in $XPKLIST;do
        check_Suite $xpk
        if [ $? -eq 0 ];then
            $CTS_DIR/tools/build/pack.py -t xpk -s $SUITE_DIR -d $TIZEN_TESTS_DIR --tools=$CTS_DIR/tools
            [ $? -ne 0 ] && echo "[wgt] <$wgt>" >> $BUILD_LOG
        fi
    done
}
    ...
    if [ $? -eq 0 ];then
        ...
        for apk in $APKLIST;do
            read -u 100
            {
                check_Suite $apk
                if [ $? -eq 0 ];then
                    $CTS_DIR/tools/build/pack.py -t apk -m $2 -a $1 -s $SUITE_DIR -d ${tests_path_arr[$2]}/$1 --tools=$CTS_DIR/tools
    ...
    if [ $? -eq 0 ];then
        ...
        for cordova in $CORDOVALIST;do
            read -u 100
            {
                ...
                check_Suite $cordova
                if [ $? -eq 0 ];then
                    $CTS_DIR/tools/build/pack.py -t cordova -s $SUITE_DIR -d $CORDOVA_TESTS_DIR/$1 --tools=$CTS_DIR/tools
...
pack_Embeddingapi(){
    ...
    if [ $? -eq 0 ];then
        ...
        for emb_suite in $EMBEDDINGLIST;do
            read -u 100
            {
                check_Suite $emb_suite
                if [ $? -eq 0 ];then
                    $CTS_DIR/tools/build/pack.py -t embeddingapi -s $SUITE_DIR -d $EMBEDDED_TESTS_DIR/$1 --tools=$CTS_DIR/tools
                    if [ $2 = "shared" ];then
                        ...
                        $CTS_DIR/tools/build/pack.py -t embeddingapi -s $SUITE_DIR -d $SHARED_TESTS_DIR/$1 --tools=$CTS_DIR/tools
