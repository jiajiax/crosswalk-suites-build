    ...
    for wgt in $WGTLIST;do
        ...
        if [ $wgt_num -eq 1 ];then
            wgt_dir=`find $CTS_DIR -name $wgt -type d`
                #recover_Tests $wgt $SUITE_DIR
                #merge_Tests $wgt $SUITE_DIR
...
pack_Apk(){
    ...
    for apk in $APKLIST;do
        ...
        #check_Suite $apk
            ...
            #recover_Tests $apk $SUITE_DIR
            #merge_Tests $apk $SUITE_DIR
    ...
    if [ $? -eq 0 ];then
        for cordova in $CORDOVALIST;do
            ...
            if [ $cordova_num -eq 1 ];then
                ...
                #recover_Tests $cordova $SUITE_DIR
                #merge_Tests $cordova $SUITE_DIR
        ...
        for aio in $AIOLIST;do
            ...
            if [ $aio_num -eq 1 ];then
                ...
                if [ $1 = "apk" ];then
                    ...
                    mv *.apk.zip ${tests_path_arr[$3]}/$2
                elif [ $1 = "cordova" ];then
                    ./pack.sh -t cordova
                    [ $? -ne 0 ] && echo "[aio] [$1] [$2] <$aio>" >> $BUILD_LOG
                    mv *.cordova.zip $CORDOVA_TESTS_DIR/$2
                fi
