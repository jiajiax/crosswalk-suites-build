...
cts_dir=$script_dir/../work_space/release/crosswalk-test-suite-12
WW_dir=$script_dir/WWbeta
demoex_dir=$cts_dir/../demo-express
log_dir=$script_dir/logs
release_commit=$log_dir/$(date +%Y-%m-%d-%T)_release
version_file=$script_dir/version_flag/Beta12_Number
pkg_tools=$cts_dir/../pkg_tools/
source $script_dir/list_suites/beta12_list
sample_list=""
...
sync_code(){
    ...
    cd $cts_dir ; git reset --hard HEAD;git checkout Crosswalk-12;git pull
...
save_package(){
    ...
    chmod -R 777 /data/pkgs/beta_WW_pkgs/$version_num"~"$end_time
    wweek=$(date +"%W" -d "+1 weeks")
    wtoday=$[$(date +%w)]
    wdir="WW"$wweek
