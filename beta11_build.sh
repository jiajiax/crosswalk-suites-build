...
cts_dir=$script_dir/../work_space/release/crosswalk-test-suite-11
WW_dir=$script_dir/WWbeta
demoex_dir=$cts_dir/../demo-express
log_dir=$script_dir/logs
version_file=$script_dir/version_flag/Beta11_Number
pkg_tools=$cts_dir/../pkg_tools
source $script_dir/list_suites/beta11_list
sample_list=""
...
webapi_usecase_pro(){
    if [ $1 = "usecase-webapi-xwalk-tests" ];then
		...
		sed -i -e '/<\/suite/d ;/<\/test_definition/d' $2/tests.android.xml && sed -n '1,/<set name="Third Party Framework/p' $demoex_dir/tests.android.xml | sed '$d' | sed '$d' | sed -n '/<set/,$p' >> $2/tests.android.xml && tail -n2 $demoex_dir/tests.android.xml >> $2/tests.android.xml
...
pack_wgt()
{
    for wgt in $WGTLIST;do
	    ...
	    webapi_usecase_pro $wgt $wgt_dir
            ...
            if [ $? -ne 0 ];then
                echo "[wgt] <$wgt> failed">>$script_dir/logs/beta11_error.log
            fi
	    cancel_xml $wgt $wgt_dir
            echo $wgt_dir
        else
...
pack_xpk()
{
    for xpk in $XPKLIST;do
        ...
        if [ $num -eq 1 ];then
            ...
            if [ $? -ne 0 ];then
                echo "[xpk] <$xpk> failed">>$script_dir/logs/beta11_error.log
            fi
            echo $xpk_dir
        else
            echo "$xpk Not unique"    
...
pack_cordova_arm()
{
    ...
    for cordova_arm in $CORDOVALIST;do
	    ...
	    webapi_usecase_pro $cordova_arm $cordova_arm_dir
            ...
            if [ $? -ne 0 ];then
                echo "[cordova][arm] <$cordova_arm> failed">>$script_dir/logs/beta11_error.log
            fi
        ...
        if [ $num -eq 1 ];then
            ...
            if [ $emb_pkg = "embedding-api-android-tests" ];then
                ...
                for emd_flag in $end_flags;do
                    ...
                    if [ $? -ne 0 ];then
                        echo "$emb_pkg failed">>$script_dir/logs/beta11_error.log
                    else
...
{
	prepare_tools $1 apk 
    cp -a $pkg_tools/crosswalk-cordova-$version_num-arm $cts_dir/tools/cordova
    for aio in $AIOLIST;do
        ...
        if [ $num -eq 1 ];then
            ...
            ./pack.sh -a $1
            if [ $? -ne 0 ];then
                echo "[webapi all-in-one][$1] <$aio_$1> failed">>$script_dir/logs/beta11_error.log
            fi
            mv *.zip $WW_dir/android/all-in-one/$1
            if [ $1 = "arm" ];then
                ./pack.sh -t cordova
                if [ $? -ne 0 ];then
                    echo "[cordova all-in-one][$1] <$aio_$1> failed">>$script_dir/logs/beta11_error.log
                fi
                mv *.zip $WW_dir/android/all-in-one/$1
            fi
            cd -
...
save_package(){
	...
	cp -a $WW_dir /data/pkgs/beta_WW_pkgs/"ww"$version_num"~"$end_time
    chmod -R 777 /data/pkgs/beta_WW_pkgs/"ww"$version_num"~"$end_time
    wweek=$(date +"%W" -d "+1 weeks")
    wtoday=$[$(date +%w)]
    wdir="WW"$wweek

    mkdir -p /mnt/otcqa/$wdir/{master/"ww"$wweek"."$wtoday,beta/"ww"$wweek"."$wtoday,stable,webtestingservice}
    cp -r /data/pkgs/beta_WW_pkgs/"ww"$version_num"~"$end_time /mnt/otcqa/$wdir/beta/"ww"$wweek"."$wtoday
...
echo "" > $script_dir/logs/beta11_error.log
init_ww
sync_code
updateVersionNum
#pack_wgt
