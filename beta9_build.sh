...
version_file=$script_dir/version_flag/Beta9_Number
pkg_tools=$cts_dir/../pkg_tools
source $script_dir/list_suites/beta9_list
...
function pack_wgt ()
{
    for wgt in $WGTLIST;do
	    ...
	    #webapi_usecase_pro $wgt $wgt_dir
            ...
            if [ $? -ne 0 ];then
                echo "$wgt failed">>$script_dir/logs/beta9_error.log
            fi
	    #cancel_xml $wgt $wgt_dir
            echo $wgt_dir
        else
...
function pack_xpk ()
{
    for xpk in $XPKLIST;do
        ...
        if [ $num -eq 1 ];then
            ...
            if [ $? -ne 0 ];then
                echo "$xpk failed">>$script_dir/logs/beta9_error.log
            fi
            echo $xpk_dir
        else
            echo "$xpk Not unique"    
...
function pack_apk_x86 ()
{
    ...
    for apk_x86 in $APKLIST;do
	    ...
	    #webapi_usecase_pro $apk_x86 $apk_x86_dir
            ...
            if [ $? -ne 0 ];then
                echo "$apk_x86 failed">>$script_dir/logs/beta9_error.log
            fi
	    #cancel_xml $apk_x86 $apk_x86_dir
            echo $apk_x86_dir
        else
...
function pack_apk_arm ()
{
    ...
    for apk_arm in $APKLIST;do
	    ...
	    #webapi_usecase_pro $apk_arm $apk_arm_dir
            ...
            if [ $? -ne 0 ];then
                echo "$apk_arm failed">>$script_dir/logs/beta9_error.log
            fi
	    #cancel_xml $apk_arm $apk_arm_dir
            echo $apk_arm_dir
        else
...
function pack_cordova_arm(){
    ...
    for cordova_arm in $CORDOVALIST;do
        ...
        if [ $num -eq 1 ];then
            ...
            if [ $? -ne 0 ];then
                echo "$cordova_arm failed">>$script_dir/logs/beta9_error.log
            fi
...
function pack_aio ()
{
    ...
    for aio in $AIOLIST;do
        ...
        if [ $num -eq 1 ];then
            ...
            ./pack.sh -a $1
            if [ $? -ne 0 ];then
                echo "$aio_$1 failed">>$script_dir/logs/beta9_error.log
            fi
...
echo "" > $script_dir/logs/beta9_error.log
