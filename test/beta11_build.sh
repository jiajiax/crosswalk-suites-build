...
version_file=$script_dir/Beta11_Number
pkg_tools=$cts_dir/../pkg_tools
source $script_dir/beta11_list
sample_list=""
...
function pack_aio ()
{
    ...
    for aio in $AIOLIST;do
        ...
        if [ $num -eq 1 ];then
            ...
            ./pack.sh -a $1
            if [ $? -ne 0 ];then
                echo "[webapi all-in-one][$1] <$aio_$1> failed">>$script_dir/error.log
            fi
            mv *.zip $WW_dir/android/all-in-one/$1
            if [ $1 = "arm" ];then
                ./pack.sh -t cordova
                if [ $? -ne 0 ];then
                    echo "[cordova all-in-one][$1] <$aio_$1> failed">>$script_dir/error.log
                fi
                mv *.zip $WW_dir/android/all-in-one/$1
            fi
            cd -
...
function save_package(){
	...
	cp -a $WW_dir $script_dir/beta_WW_list/"ww"$version_num"~"$end_time
    chmod -R 777 $script_dir/beta_WW_list/"ww"$version_num"~"$end_time
    wweek=$(date +"%W" -d "+1 weeks")
    wtoday=$[$(date +%w)]
    wdir="WW"$wweek

    mkdir -p /mnt/otcqa/$wdir/{master/"ww"$wweek"."$wtoday,beta/"ww"$wweek"."$wtoday,stable,webtestingservice}
    cp -a $script_dir/beta_WW_list/"ww"$version_num"~"$end_time /mnt/otcqa/$wdir/beta/"ww"$wweek"."$wtoday/
